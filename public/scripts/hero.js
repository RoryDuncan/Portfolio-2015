...
var composites, randomColor, randomComposite, renderImagesAs3by3, run, shuffle;
...
composites = ["color-dodge", "color-burn", "overlay", "exclusion", "hue"];

randomComposite = function() {
  var n;
...
renderImagesAs3by3 = function(canvas, images) {
  ...
  for (i = _i = 0, _len = images.length; _i < _len; i = ++_i) {
    ...
    _results.push((function(image, i) {
      ...
      return canvas.globalCompositeOperation("source-over").drawImage(image, padding + x, padding + y * height, width, height).globalCompositeOperation(randomComposite()).fillStyle(randomColor()).fillRect(padding + x, padding + y * height, width, height).globalCompositeOperation("source-over");
    })(image, i));
  }
  return _results;
};
...
run = function() {
  ...
  complete = function() {
    ...
    canvas.canvas.addEventListener("mouseover", function() {
      return changes = 2 + ~~(Math.random() * 10);
    });
  };
  loadImage = function() {
    loaded++;
